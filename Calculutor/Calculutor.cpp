				...
				if ((mousePos.x >= key.getPosition().x) &&
					...
					if (event.type == sf::Event::MouseButtonReleased) {
						if (key.getText()[0] >= '0' && key.getText()[0] <= '9' ||
							...
							if (key.getText()[0] == '=') {
								if (operation != "") {
									currenNum = stof(fieldText);
									std::cout << operation;
									if (operation == "-") {
										fieldText = std::to_string(prevNum - currenNum);
									}
									if (operation == "*") {
										fieldText = std::to_string(prevNum * currenNum);
									}
									...
									if (operation == "X^") {
										fieldText = std::to_string(pow(prevNum,currenNum));
									}
									if (operation == "sqrt") {
										fieldText = std::to_string(sqrt(prevNum));
									}
									if (operation == "+") {
										fieldText = std::to_string(prevNum + currenNum);
									}
